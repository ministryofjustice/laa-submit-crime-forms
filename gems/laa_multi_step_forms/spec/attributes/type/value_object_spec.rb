...
RSpec.describe Type::ValueObject do
  ...
  context 'when array flag is not set' do
  ...
  context 'when array flag is set' do
    subject { described_class.new(source: YesNoAnswer, array: true) }

    describe 'when value is an array of symbols' do
      let(:value) { [:yes] }

      it { expect(coerced_value).to eq([YesNoAnswer::YES]) }
    end

    describe 'when value is an array of strings' do
      let(:value) { ['yes'] }

      it { expect(coerced_value).to eq([YesNoAnswer::YES]) }
    end

    describe 'when value is already an array of value-objects' do
      let(:value) { [YesNoAnswer::YES] }
    ...
    describe '#serialize' do
      ...
      it 'casts a value from the ruby type to a json string' do
        expect(
          subject.serialize(value)
        ).to eq('["no"]')
      end
    ...
    describe 'identity methods' do
      let(:type_base) { described_class.new(source: String) }
      let(:type_one) { described_class.new(source: [type_base], array: true) }
      let(:type_two) { described_class.new(source: [type_base], array: true) }
      let(:type_three) { described_class.new(source: [type_base, type_base], array: true) }

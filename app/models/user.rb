class User < ApplicationRecord
  devise :lockable, :timeoutable, :trackable,
         :omniauthable

  ROLES = [
    CASEWORKER = 'caseworker'.freeze,
    SUPERVISOR = 'supervisor'.freeze,
    PROVIDER = 'provider'.freeze
  ].freeze

  include AuthUpdateable
  include Reauthable

  validates :role, inclusion: { in: ROLES }

  scope :active, -> { where(deactivated_at: nil).where.not(auth_uid: nil) }
  scope :pending_activation, -> { where(auth_uid: nil, deactivated_at: nil) }
  scope :assessors, -> { where(role: [CASEWORKER, SUPERVISOR]) }

  def supervisor?
    role == SUPERVISOR
  end

  def pending_activation?
    auth_uid.nil? && first_auth_at.nil?
  end

  store_accessor :settings,
                 :selected_office_code,
                 :legal_rep_first_name,
                 :legal_rep_last_name,
                 :legal_rep_telephone

  def display_name
    if first_name.present? && last_name.present?
      "#{first_name} #{last_name}"
    else
      email
    end
  end

  def multiple_offices?
    office_codes.size > 1
  end

  class << self
    def from_omniauth(auth)
      find_or_initialize_by(auth_provider: auth.provider, auth_uid: auth.uid).tap do |record|
        record.update(
          email: auth.info.email,
          description: auth.info.description,
          roles: auth.info.roles,
          office_codes: active_office_codes(auth),
          role: PROVIDER,
        )

        ensure_default_office(record)
      end
    end

    private

    # If `selected_office_code` is nil or unknown, and there is
    # only one office returned, it defaults to that office.
    # If there are more offices, the provider will choose one.
    def ensure_default_office(record)
      return if record.office_codes.include?(record.selected_office_code)

      record.update(
        selected_office_code: (
          record.office_codes.first unless record.multiple_offices?
        )
      )
    end

    def active_office_codes(auth)
      Providers::ActiveOfficeChecker.new(auth.info).active_office_codes
    end
  end
end

...
module Steps
  class DefendantsDetailsForm < Steps::BaseFormObject
    ...
    def defendants
      @defendants ||= defendants_collection
                      .map { |defendant| DefendantDetailsForm.new(defendant.merge(application:)) }
                      .tap { |defendants| defendants << main_defendant if defendants.none? }
    end

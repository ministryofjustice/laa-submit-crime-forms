module Decisions
  class BackDecisionTree < DslDecisionTree
    ...
    from(NSM_FIRM_DETAILS).goto(show: 'nsm/steps/start_page')
    from('nsm/steps/defendant_details')
      .when(-> { application.defendants.exists? })
      .goto(edit: NSM_DEFENDANT_SUMMARY)
      .goto(edit: NSM_FIRM_DETAILS)
    from(NSM_DEFENDANT_SUMMARY).goto(edit: NSM_FIRM_DETAILS)
    from('nsm/steps/defendant_delete').goto(edit: NSM_DEFENDANT_SUMMARY)
    from('nsm/steps/case_details').goto(edit: NSM_DEFENDANT_SUMMARY)
    ...
    from(NSM_CLAIM_DETAILS).goto(edit: 'nsm/steps/reason_for_claim')
    from(NSM_WORK_ITEM)
      .when(-> { application.work_items.exists? })
      .goto(edit: NSM_WORK_ITEMS)
      .goto(edit: NSM_CLAIM_DETAILS)
    from(NSM_WORK_ITEMS).goto(edit: NSM_CLAIM_DETAILS)
    from('nsm/steps/work_item_delete').goto(edit: NSM_WORK_ITEMS)
    from(NSM_LETTERS_CALLS)
      .when(-> { application.work_items.exists? })
      .goto(edit: NSM_WORK_ITEMS)
      .goto(edit: NSM_WORK_ITEM, work_item_id: Nsm::StartPage::NEW_RECORD)
    from(NSM_DISBURSEMENT_ADD).goto(edit: NSM_LETTERS_CALLS)
    from(NSM_DISBURSEMENT_TYPE)
    ...
    from('nsm/steps/disbursement_cost')
      .goto(edit: NSM_DISBURSEMENT_TYPE, disbursement_id: -> { record.id })
    from(NSM_DISBURSEMENTS).goto(edit: NSM_LETTERS_CALLS)
    from('nsm/steps/disbursement_delete').goto(edit: NSM_DISBURSEMENTS)
    ...
    from(NSM_EQUALITY).goto(show: 'nsm/steps/check_answers')
    from('nsm/steps/equality_questions').goto(edit: NSM_EQUALITY)
    # TODO: we should be storing the answer to the ask equality question and use that in the decision
    from('nsm/steps/solicitor_declaration').goto(edit: NSM_EQUALITY)

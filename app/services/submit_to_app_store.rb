class SubmitToAppStore < ApplicationJob
  ...
  def notify(submission)
    Nsm::SubmissionMailer.notify(submission).deliver! if submission.is_a?(Claim)
    ::PriorAuthority::SubmissionMailer.notify(submission).deliver! if submission.is_a?(PriorAuthorityApplication)
  end
end

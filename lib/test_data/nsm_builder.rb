module TestData
  class NsmBuilder
    ...
    def errors_for(task_klass, claim)
      task = task_klass.new(application: claim)
      form = task.send(:associated_form).build(task.send(:record), application: claim)
      form.valid?
      form.errors
    end
    ...
    def options
      {
        magistrates: [
          ...
          proc { { disbursements_count: rand(25), work_items_count: rand(50) } }
        ],
        breach: [
        ...
        no_disburesments: [
          [:claim, :complete, :case_type_magistrates, :build_associates],
          proc { { disbursements_count: 0, work_items_count: rand(50) } }
        ],
        enhanced_rates: [

...
```sh
SLEEP=false PROVIDER_MODE=generated bin/rails 'submit_dummy_data:bulk_prior_authority[1000,2025,20,200,3]'
...
Without `PROVIDER_MODE=generated`, local/development bulk tasks use the primary dev login provider and its office
codes so generated records are visible when logging in as the primary test user.

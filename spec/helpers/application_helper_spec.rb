...
RSpec.describe ApplicationHelper, type: :helper do
  describe 'current_office_code' do
    before do
      allow(helper).to receive(:current_user).and_return(provider)
    end
    ...
    context 'when selected_office_code is present' do
      let(:provider) { instance_double(User, selected_office_code: 'A1') }
    ...
    context 'when no selected_office_code but office_codes are present' do
      let(:provider) { instance_double(User, selected_office_code: nil, office_codes: %w[A2 A3]) }
    ...
    context 'when no selected_office_code or office_codes are present' do
      let(:provider) { instance_double(User, selected_office_code: nil, office_codes: []) }

...
RSpec.describe Assess::ClaimDetailsController do
  context 'show' do
    let(:caseworker) { create(:caseworker) }
    let(:claim) { instance_double(SubmittedClaim, id: claim_id) }
    let(:claim_id) { SecureRandom.uuid }
    let(:claim_summary) { instance_double(Assess::V1::ClaimSummary) }
    let(:claim_details) { instance_double(Assess::ClaimDetails::Table) }

    before do
      allow(SubmittedClaim).to receive(:find).and_return(claim)
      allow(Assess::BaseViewModel).to receive(:build).and_return(claim_summary)
      allow(Assess::ClaimDetails::Table).to receive(:new).and_return(claim_details)
    end

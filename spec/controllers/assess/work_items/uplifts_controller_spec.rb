...
RSpec.describe Assess::WorkItems::UpliftsController do
  let(:caseworker) { create(:caseworker) }

  context 'edit' do
    let(:claim) { instance_double(SubmittedClaim, id: claim_id, risk: 'high') }
    let(:claim_id) { SecureRandom.uuid }
    let(:form) { instance_double(Assess::Uplift::WorkItemsForm) }

    before do
      allow(SubmittedClaim).to receive(:find).and_return(claim)
      allow(Assess::Uplift::WorkItemsForm).to receive(:new).and_return(form)
    end
  ...
  context 'update' do
    let(:claim) { instance_double(SubmittedClaim, id: claim_id, risk: 'high') }
    let(:claim_id) { SecureRandom.uuid }
    let(:form) { instance_double(Assess::Uplift::WorkItemsForm, save:) }
    ...
    context 'when form save is successful' do
      ...
      it 'renders successfully with claims' do
        allow(controller).to receive(:render)
        put :update, params: { claim_id: claim_id, assess_uplift_work_items_form: { some: :data } }

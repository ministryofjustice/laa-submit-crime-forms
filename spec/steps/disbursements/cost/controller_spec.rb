...
RSpec.describe Steps::DisbursementCostController, type: :controller do
  let(:disbursement) { existing_case.is_a?(Claim) ? existing_case.disbursements.create : nil }

  it_behaves_like 'a generic step controller', Steps::DisbursementCostForm, Decisions::SimpleDecisionTree,
                  ->(scope) { { disbursement_id: scope.disbursement&.id || '4321' } }
  it_behaves_like 'a step that can be drafted', Steps::DisbursementCostForm,
                  ->(scope) { { disbursement_id: scope.disbursement&.id || '4321' } }
  ...
  describe '#edit' do
    ...
    context 'and disbursement exists' do
      ...
      it 'passes the existing disbursement to the form' do
        allow(Steps::DisbursementCostForm).to receive(:build)
        expect do
          get :edit,
        ...
        expect(Steps::DisbursementCostForm).to have_received(:build).with(disbursements.first, application:)
      end
    end
